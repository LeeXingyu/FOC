...
{
	g_pCiA402Axis = pCiA402Axis;

	/* IncFactor: convert a velocity (inc/s) into the position increment of one cycle.
	       0.0010922 ~= 1/915.5; u32CycleTime is the DC cycle time in [us].
	       => IncFactor = velocity * IncFactor gives the increment for this cycle. */
	float IncFactor    = (float)0.0010922 * (float) pCiA402Axis->u32CycleTime;

	/* Position reached when |TargetPosition - ActualPosition| is within this window [inc] */
	INT32 i32PositionWindow = 1;
...
//			if (g_axis.enCtrlMode != CTRL_MODE_POSITION && pCiA402Axis->Objects.objTargetPosition != 0)
//			{
...
//			{
//				float fPosRef = (float)pCiA402Axis->Objects.objTargetPosition  / 10000.0f;
//
//				MC_Set_Position_Angle_Reference(fPosRef * 10);
//
//				/* 实际位置：iAbsRawPos - iZeroAngle 是 counts，再缩放回 1 圈 = 10000 */
//				int32_t iCounts = (float)(g_axis.posCtrl.iAbsRawPos - g_axis.posCtrl.iZeroAngle) / ENCODER_COUNT * 360.0;
//				pCiA402Axis->Objects.objPositionActualValue = (INT32)((float)iCounts * 10000.0f / 10.0f);
//			}
//			else
//			{
...
//        	break;
        case CYCLIC_SYNC_POSITION_MODE:
            // 切换到位置模式
        	float fPosRef = (float)pCiA402Axis->Objects.objTargetPosition  / 10000.0f * 10.0f;
            if (g_axis.enCtrlMode != CTRL_MODE_POSITION && pCiA402Axis->Objects.objTargetPosition != 0)
            {
            	MC_Set_Control_Mode(CTRL_MODE_POSITION);

//        		printf("objTargetPosition: %d %f\n", pCiA402Axis->Objects.objTargetPosition, fPosRef);

//            	MC_Set_Position_Angle_Reference(fPosRef);
			...
			{
//				MC_Set_Position_Angle_Reference(fPosRef);
        		g_axis.posCtrl.fPosRef = (fPosRef) * ENCODER_COUNT / 360.0f;
				float fCounts = (float)(g_axis.posCtrl.iAbsRawPos - g_axis.posCtrl.iZeroAngle) / ENCODER_COUNT * 360.0;
			    pCiA402Axis->Objects.objPositionActualValue = (INT32)(fCounts * 10000.0f / 10.0f);
			}
            else
			/* 实际位置：iAbsRawPos - iZeroAngle 是 counts，再缩放回 1 圈 = 10000 */
			{
...
//            if(IncFactor != 0)
//				i32TargetVelocity = (INT32)((pCiA402Axis->Objects.objTargetPosition
//										  - pCiA402Axis->Objects.objPositionActualValue) / IncFactor);
//			else
//				i32TargetVelocity = 0;
    ...
    {
        pCiA402Axis->Objects.objStatusWord |= STATUSWORD_INTERNAL_LIMIT;//over limit
    }

    /* ============ 上报反馈 ============ */
    /*report the actual velocity (0x606C)*/
//	pCiA402Axis->Objects.objVelocityActualValue = i32TargetVelocity;

	/*------------------------------------------------------------------
	  Target reached (0x6041 bit10):
	  set when the actual position is inside the position window around
	  the target position, cleared while still moving towards it.
	------------------------------------------------------------------*/
//	{
//		INT32 i32Error = pCiA402Axis->Objects.objTargetPosition - pCiA402Axis->Objects.objPositionActualValue;
//		if(i32Error < 0)
//			i32Error = -i32Error;
//
//		if(i32Error <= i32PositionWindow)
...
//                    pTmpData += SIZEOF(TCiA402PDO1A00);
                ...
                //total 12bytes		
                *pTmpData++ = LocalAxes[AxisIndex].Objects.objStatusWord;//2bytes
                *pTmpData++ = LocalAxes[AxisIndex].Objects.objPositionActualValue & 0xFFFF;//4bytes
                *pTmpData++ = LocalAxes[AxisIndex].Objects.objPositionActualValue>>16;
                *pTmpData++ = LocalAxes[AxisIndex].Objects.objVelocityActualValue & 0xFFFF;//4bytes
                *pTmpData++ = LocalAxes[AxisIndex].Objects.objVelocityActualValue>>16;
                *pTmpData++ = LocalAxes[AxisIndex].Objects.objModesOfOperationDisplay & 0xFF;//2bytes    
...
//                pTmpData += SIZEOF(TCiA402PDO1600);
                ...
                //total 12bytes
                LocalAxes[AxisIndex].Objects.objControlWord = SWAPWORD(*pTmpData++);//2bytes
                LocalAxes[AxisIndex].Objects.objTargetPosition =SWAPWORD(*pTmpData++);//4bytes
				LocalAxes[AxisIndex].Objects.objTargetPosition+=(SWAPWORD(*pTmpData++)<<16);	
                LocalAxes[AxisIndex].Objects.objTargetVelocity =SWAPWORD(*pTmpData++);//4bytes
				LocalAxes[AxisIndex].Objects.objTargetVelocity+=(SWAPWORD(*pTmpData++)<<16);							
                LocalAxes[AxisIndex].Objects.objModesOfOperation = SWAPWORD(*pTmpData++)&0xff;	//2bytes			
			}
...
void APPL_Application(void)
{
    ...
    for(i = 0; i < 1 ;i++)//MAX_AXES,1 axis for test
    {
        if(LocalAxes[i].bAxisIsActive)
